...
"""
Se crea la función InicializarPoblacion encargada de generar una población inicial de forma aleatoria
...
"""
def InicializarPoblacion(tamañoPoblacion, nGenes):
    ...
    for i in range(tamañoPoblacion):
        ...
        for j in range (nGenes):
            #Se crea un gen aleatorio que no esté ya agregado
            nAleatorio = int(random.choice([i for i in range(0,nGenes) if i not in lista_Ciudades]))
...
poblacion = InicializarPoblacion(tamañoPoblacion, nGenes)
...
for i in range(nGeneraciones):
    ...
    #Se mutan todos los genes para así tener la siguiente generación
...
#Reviso la posición del mejor cromosoma, el que tiene el mejor ajuste, en la población
...
#Revisa cual generacion tiene el mejor valor de ajuste de entre todos
...
ax.set_title('PVA valores ajuste generaciones')
...
np.savetxt('caminoMásCorto_AGE.txt',coordenadas_mejor_cromosoma)
